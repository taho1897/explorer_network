...
public class SocketThread extends Thread {
	...
	public void run() {
		try {// 외곽 try
			...
			while(true) {// 계속 반복
				try {// 내각 try
					explorer.selectMenu(menuSelect, root, this);// 메뉴선택 메소드 호출(선택 메뉴, root 객체, 소켓)
	...
	public void selectMenu(int menuSelect, Component componet, SocketThread socketThread) throws UnsupportedOperationException, IOException {
		...
		size = in.read(byteArray);// 인풋스트림으로 받은 내용의 크기를 바이트배열을 이용하여 저장
		if(size <= 0) {// 내용 크기가 0이하일 때 IO exception 발생
	...
	public void sendMessage(String message) {
		try {// try
			PrintWriter out = new PrintWriter(new BufferedWriter(new OutputStreamWriter(socket.getOutputStream())), true);// 소켓의 OutputStream을 이용하여 PrintWriter 객체 선언
